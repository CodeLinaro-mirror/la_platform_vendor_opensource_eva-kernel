		...
		void **new_session)
{
	...
	if (id < 0x7FFF0000) {
	     dprintk(CVP_ERR,
		"%s: idr allocation failed or out of range for session %pK of inst %pK, id=%d\n",
		__func__, s, session_id, id);
	     goto err_session_init_fail;
	}
...
err_session_init_fail:
    if (s) {
        /* Clean up IDR allocation if it was successful */
        if (inst->sess_id != 0) {
            mutex_lock(&core->idr_mtx);
            idr_remove(&core->sess_idr, inst->sess_id);
            mutex_unlock(&core->idr_mtx);
        }
	inst->sess_id = 0;
        __session_clean(s);
